...
void setup() {
    ...
    if (err) DBG_FMT("[sensor] stopPeriodicMeasurement err=%u (ok if idle)\n", err);
    delay(600);
    i2c_reset();

    err = scd4x.measureSingleShot();
    sensor_ok = (err == 0);
    if (sensor_ok) DBG("[sensor] SCD41 ok — waiting for first reading (~5 s)");
    else           DBG_FMT("[sensor] SCD41 unreachable — err=%u\n", err);

    // ── Display ───────────────────────────────────────────────────────────────
    display_ok = display_init();
    if (display_ok) display_show_message("Cognitiv");
...
void loop() {
    ...
    // Check button again — a press during the ~5 s blocking sensor read is caught here.
    handle_button();

    if (display_ok) display_show_co2(co2);
