...
void setup() {
  ...
  if (voltage > 0.1f && voltage < MIN_OPERATING_VOLTAGE) {
    ...
    DBG("LOW BATTERY %.2f V < %.2f V → emergency sleep", voltage,
...
static void handleNormalCycle() {
  // ── I2C Bus Recovery ──────────────────────────────────────────────
  // SIMPLIFICATION: Removed manual bit-bang recovery and scanning.
  // Rely on standard Wire.begin() and library timeouts.
  I2CManager::init();

  // Give the bus + sensor time to settle after init.
  delay(100);
  ...
  if (now > 1700000000L) { // Only if NTP succeeded
    ...
    if (PowerManager::isQuietHours(ti.tm_hour, ti.tm_min)) {
      DBG("Inside quiet window  %02d:%02d → entering quiet mode", ti.tm_hour,
          ti.tm_min);
      enterQuietMode();
      return; // unreachable
    }
...
static void enterQuietMode() {
  ...
  DBG("Quiet mode: %lu s total, %u chunks of %lu s", total_sec, chunks,
      MAX_DEEP_SLEEP_SEC);
